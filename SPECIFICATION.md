...
│   └── test/
│       ├── testUtils.ts             # composable テスト用マウントヘルパ
│       ├── composables/             # composables の Vitest 単体テスト
│       └── utils/                   # utils の Vitest 単体テスト
│
...
│   └── capabilities/
│       ├── default.json             # メインウィンドウのセキュリティ権限設定
│       └── viewer.json              # ビューア/スライドショーウィンドウの権限設定
│
├── public/                           # 静的アセット
│   ├── ageha.svg                    # アプリアイコン
...
| パッケージ | バージョン | 用途                               |
| ---------- | ---------- | ---------------------------------- |
| `vitest`   | ^3.2.4     | フロントエンド単体テスト           |
| `vue-tsc`  | ^2.1.10    | Vue / TypeScript 型チェック        |
| `vite`     | ^6.0.3     | 開発サーバー / ビルド / テスト基盤 |
...
| `base64`                         | ^0.22       | ローカル画像の data URL 化                                |
...
│  │  ├─ ageha.css        (Markdown 用 CSS)   │              │
│  │  └─ ageha-slide.css  (Slides 用 CSS)     │              │
│  └──────────────────────────────────────────┘              │
└────────────────────────────────────────────────────────────┘
```
...
3. main.rs: ageha.css / ageha-slide.css / ageha.env.json が不足していれば既定値で生成
4. main.rs: コマンドライン引数解析
5. main.rs: tracing ロガー初期化
6. main.rs: lib::run() に設定パス (args_file_path, css_file_path, slide_css_file_path) を直接渡す
7. lib.rs: AppConfig を Tauri State として登録、プラグイン登録
8. main.ts: Pinia ストア初期化 (appInits → Rust から起動引数取得)
9. main.ts: Markdown 用 CSS を DOM に適用し、Slides 用 CSS はプレビュー/出力生成時に注入
...
- `markdown` モード:
  ...
  - ローカル画像は preview / export / print の差異を避けるため、事前に `data:` URL へ埋め込む。
  - プレビューはアプリ DOM 上に `v-html` で直接描画する。
  - エディタからプレビューへのスクロール同期はツールバートグルが ON のときに有効である。
- `slides` モード:
  - 文書先頭 frontmatter の `marp: true` を検出した場合のみ有効化する。
  - `@marp-team/marp-core` により HTML/CSS を生成し、Mermaid は SVG へ事前変換する。
  - ローカル画像は `srcdoc` / 別ウィンドウ / 印刷でも失われないよう、事前に `data:` URL へ埋め込む。
...
| `save_temp_html`            | `html: String`                                  | `String`            | HTML を `~/.ageha/viewer_<ms>.html` に書き出しパスを返す |
| `delete_file`               | `path: String`                                  | なし                | 指定ファイルを削除（一時ファイルのクリーンアップ用）     |
| `read_binary_file_data_url` | `target_file: String`                           | `String`            | ローカル画像を Base64 の data URL 化                     |
...
AppConfig {                    // Tauri State として管理
    ...
    slide_css_file_path: String, // Slides 用 CSS ファイルパス
}
...
LaunchRequestData {
    ...
    css_data: String,          // Markdown 用 CSS データ
    slide_css_data: String,    // Slides 用 CSS データ
    // ヘルパー: error(msg, css_data, slide_css_data)
}
...
| `useExport`            | `composables/useExport.ts`            | 印刷/PDF出力、HTMLエクスポート、ビューア/スライドショーウィンドウ起動（WebviewWindow ベース） |
...
| `rustArgsData.css_data`       | `string` | Markdown 用 CSS        |
| `rustArgsData.slide_css_data` | `string` | Slides 用 CSS          |
...
| `isScrollSyncFromLocalStorage` | `boolean` | `true`     | スクロール同期の有効/無効     |
...
| 画像処理       | ローカル画像は絶対/相対パスともに解決し、プレビュー系では `data:` URL 化            |
| レイアウト     | エディタとプレビューの分割表示 (50/50)                                              |
| Slides 分離    | `slides` モード時は iframe にプレビューを分離                                       |
| スクロール同期 | エディタ → プレビュー方向の同期 (`markdown` モード時かつ Sync トグル ON のときのみ) |
| トグル         | ツールバーの `Sync` ボタン                                                          |
...
| math 設定        | `katex` に固定                                                                |
| プレビュー       | iframe 内に独立描画                                                           |
| ユーザー CSS     | `~/.ageha/ageha-slide.css` を built-in テーマの後段で適用                     |
...
- `---` のみではスライドモードに移行しない。
- `ageha-slide.css` は preview / open-viewer / export HTML / print に共通適用される。
- ユーザー CSS は Marp の DOM 構造に合わせて内部で再スコープ化し、`section` や `h1` のような自然なセレクタでも上書きできるようにする。
...
- **画像:** 絶対/相対パス解決、幅指定サポート (`![alt](path =200)`)、ローカル画像の `data:` URL 埋め込み
...
- `markdown` モードでは CSS・Mermaid・ローカル画像 data URL をインラインで埋込
- `slides` モードでは Marp 生成済みの HTML/CSS・ローカル画像 data URL を単一 HTML に埋込
- ファイル保存ダイアログで保存先指定

#### 別ウィンドウ表示 (`Ctrl+Alt+W`)

- 現在のプレビュー内容を Tauri の WebviewWindow（アドレスバーなし）で表示
- HTML を `~/.ageha/viewer_<ms>.html` に一時保存して開き、ウィンドウを閉じると自動削除
- `markdown` モードでは CSS・Mermaid・KaTeX をすべてインライン化したスタンドアロン HTML を使用
- `slides` モードでは preview と同一のスライド用 HTML をそのまま表示
...
- スライドサイズに合わせて `transform: scale()` でウィンドウにフィット
...
| `ageha.css`        | `~/.ageha/ageha.css`        | Markdown 用カスタムスタイルシート                                     |
| `ageha-slide.css`  | `~/.ageha/ageha-slide.css`  | Slides 用カスタムスタイルシート                                       |
| `viewer_<ms>.html` | `~/.ageha/viewer_<ms>.html` | ビューア/スライドショー用一時ファイル（ウィンドウを閉じると自動削除） |
| `settings.json`    | OS 固有のデータディレクトリ | ユーザー設定 (Tauri Store)                                            |
...
| プロパティ            | 型       | 説明                           |
| --------------------- | -------- | ------------------------------ |
| `css_file_path`       | `string` | Markdown 用 CSS ファイルのパス |
| `slide_css_file_path` | `string` | Slides 用 CSS ファイルのパス   |
...
スライド文書専用のカスタムスタイルシート。デフォルトではコメント付きのサンプルを生成し、必要に応じて `section`、`section.lead`、`h1`、`img`、`.mermaid-slide` などを上書きできる。

注記:

- `ageha-slide.css` は `slides` モードの preview / open-viewer / export HTML / print へ共通適用される。
- ユーザー CSS は Marp が出力する DOM 構造に合わせて内部で再スコープ化されるため、自然なセレクタ記述でも built-in テーマを上書きしやすい。
...
cd src-tauri && cargo test  # Rust バックエンド単体テスト
```
...
| 領域              | ツール     | 対象                                                       |
| ----------------- | ---------- | ---------------------------------------------------------- |
| フロントエンド    | Vitest     | `src/test/**/*.test.ts`。composables と utils の単体テスト |
| Rust バックエンド | Cargo test | パス正規化、ファイル I/O 補助、MIME 判定、IPC レスポンス型 |

- フロントエンドのテストは Tauri API を直接呼び出す部分をモックし、純粋関数に近い単位で検証する。
- Rust 側のテストは Tauri ランタイムを起動せず、標準ライブラリで検証できる関数とデータ構造を対象とする。
- ビルド前の基本確認として `npm test` と `cargo test` を実行する。
...
| バージョン | 内容                                                         |
| ---------- | ------------------------------------------------------------ |
| v0.3.0     | 英語 UI 対応、表示言語設定の永続化、スクロール同期トグル追加 |
